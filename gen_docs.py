# -*- coding: utf-8 -*-
"""JobSearchSF - clean professional document writer + subpage generator.

Regenerates, for EVERY job row in assets/js/data.js:
  * assets/resume/job-NN_resume.txt / .pdf
  * assets/cover/job-NN_cover.txt / .pdf
  * assets/cover/job-NN_email.txt
  * assets/resume/Brian_Chemistry_Resume.txt / .pdf   (generic master copy)
  * jobs/job-NN.html                                  (10-step apply guide)

Rules enforced here (from Brian's instructions):
  - resumes / cover letters / emails contain NO transportation or commute content
    (transit lives only in the "Getting there" section of each subpage)
  - no "tailored for you" meta-language, no match-score footers, no placeholders
    like [Insert Date] / [Add if you have] / [owner].github.io
  - only facts that appear in the master resume are claimed; posting keywords may
    re-order or re-label real experience but never add techniques Brian has not done
"""
import os
import re
import sys
...
def load_jobs():
    ...
    body = body.split("[", 1)[1]
    jobs = []
    for raw in re.findall(r"\{\s*id: \"(job-\d+)\".*?\n  \}", body, re.S):
        pass
...
# ------------------------------------------------------- keyword -> real skills
# Every label below is something Brian actually did (see master resume facts).
SKILL_LINES = [
...
def role_type(job):
    """Short, human role name: text before the first dash, trimmed of noise."""
...
def greeting_for(job):
    comp = (job.get("company", "") + " " + job.get("position", "")).lower()
    if "ucsf" in comp or "university of california" in comp or "aprecruit" in job.get("officialLink", ""):
        return "Dear Search Committee,"
    if "city & county" in comp or "careers.sf.gov" in job.get("officialLink", ""):
        return "Dear Hiring Manager,"
    if "san francisco state" in comp or "university of san francisco" in comp or "city college" in comp:
        return "Dear Selection Committee,"
    return "Dear Hiring Manager,"


def recipient_block(job):
    """Company / department / address lines for the letter - only what is verified."""
...
def order_experience(hay):
    """Re-order real experience blocks so the most relevant one leads (no new claims)."""
    def score(role):
        s = 0
        for b in role[3]:
...
def base_cover_text():
    """Generic, ready-to-edit base letter - no company name, no placeholders, no transit."""
  ...
  <main class="wrap">
    <section class="block" id="apply">
      <div class="card">
        ...
        <p class="sec-sub">Ten short steps. Everything is click, copy, paste. You never pay anything and you never need an agency.</p>
        @@STATUS_CALL@@
...
@@STEPS@@
        ...
        <div class="callout blue">
          <strong>Copy-paste tip:</strong> use the Copy button on each document below, then paste straight into the
          application form. The resume and cover letter are ordinary application documents - they do not mention how
          you get to work or this website.
        </div>
    ...
    <section class="block" id="resume">
      <div class="card">
        ...
        <p class="sec-sub">One page. Download the PDF, or copy the text if the form only takes plain text.</p>
        <p>
          <a href="../assets/resume/@@ID@@_resume.pdf" class="btn btn-primary" download>Download resume PDF</a>
          <a href="../assets/resume/@@ID@@_resume.txt" class="btn" download>Resume as TXT</a>
    ...
    <section class="block" id="cover">
      <div class="card">
        ...
        <p class="sec-sub">Three short paragraphs - long enough to be taken seriously, short enough to be read.</p>
        <p>
          <a href="../assets/cover/@@ID@@_cover.pdf" class="btn btn-primary" download>Download cover letter PDF</a>
          <a href="../assets/cover/@@ID@@_cover.txt" class="btn" download>Cover letter as TXT</a>
        </p>
        <div class="copy-box">
    ...
    <section class="block" id="email">
      <div class="card">
        ...
        <p class="sec-sub">Use only if the official posting lists a contact address, or if the portal has a message box. Otherwise the portal application is enough.</p>
        <p><a href="../assets/cover/@@ID@@_email.txt" class="btn" download>Download email as TXT</a></p>
        <div class="copy-box">
    ...
    <section class="block" id="transit">
      <div class="card">
        ...
        <p class="sec-sub">SFMTA only - kept off the resume on purpose. This section is for you, not the hiring manager.</p>
        <div class="callout">
          <strong>Home:</strong> 21st Ave &amp; Judah St, Inner Sunset, San Francisco 94122 (Judah &amp; 21st Ave stop, N Judah)<br>
    ...
    <section class="block" id="verify">
      <div class="card">
        ...
        <p class="sec-sub">Nothing here is inferred. Where the official source did not say something, this page says so instead of guessing.</p>
        <ul class="clean">
@@VERIFY@@
        </ul>
        ...
        <p><a href="../index.html" class="btn">&larr; Back to all @@COUNT@@ jobs</a> <a href="@@OFFICIAL@@" target="_blank" rel="noopener" class="btn btn-primary">Open official careers page</a></p>
      </div>
    </section>
    ...
    <section class="block" id="tracker">
      <div class="card">
        ...
        <p class="sec-sub">Marked steps are stored in your browser only - nothing is uploaded anywhere.</p>
        <div id="tracker-widget-@@ID@@"></div>
      </div>
    </section>

    <footer>
      <p>JobSearchSF - @@ID@@ - @@COMPANY_ESC@@ - verification snapshot <span id="today"></span>. Apply through the official employer portal only. No recruiters, no fees, no guesses.</p>
      <p><a href="../index.html">&larr; Back to main list</a> | <a href="@@OFFICIAL@@" target="_blank" rel="noopener">Official careers page</a> | <a href="@@APPLY_URL@@" target="_blank" rel="noopener">Apply direct</a></p>
    </footer>
  </main>
...
def build_steps(job):
    ...
    rt = role_type(job)
    monitor = job.get("status") != "recently-posted"
    portal = job.get("applyChannel", "official employer portal")
    ...
    steps.append("<strong>Read the qualifications once, slowly (3 min).</strong> Highlight what you have done "
                 "(instrument names, sample types, documentation habits) and what you have not. Being honest about the "
                 "second list in an interview is worth far more than padding the CV.")
    steps.append("<strong>Create an account on the official portal (3 min).</strong> Use Brian.j1274@gmail.com and a "
                 "password manager entry named for this employer. Legitimate domains here: job-boards.greenhouse.io, "
                 "jobs.ashbyhq.com, jobs.lever.co, aprecruit.ucsf.edu, careers.sf.gov, recruiting.ultipro.com.")
    steps.append("<strong>Download the resume and cover letter (1 min).</strong> Buttons are in the Resume and Cover "
                 "letter sections below; each also has a one-click Copy button if the form only accepts pasted text.")
    steps.append("<strong>Fill the personal details (5 min).</strong> Name, San Francisco CA 94122, (707) 596-8503, "
                 "Brian.j1274@gmail.com. For &quot;How did you hear about us?&quot; choose &quot;Company website&quot; "
                 "or &quot;Other&quot;. Work authorization: authorized to work in the US; no sponsorship required.")
    steps.append("<strong>Upload documents (2 min).</strong> Upload the PDF resume, then the cover letter if the form "
                 "asks. Keep the file names as they are - they identify you cleanly.")
    steps.append("<strong>Answer screening questions truthfully (5 min).</strong> The role&#39;s stated core is: %s. "
                 "If a question asks about a technique you have not run, answer no and mention what you have run "
                 "instead - a clean &#39;I have not, but I learn instruments fast&#39; beats a padded yes."
                 % esc(req_short))
    steps.append("<strong>Add references (2 min).</strong> Two or three: a supervisor or PI (Quintara, MicroConstants, "
                 "Threshold or the UC Santa Cruz research lab) with name, title, relationship, email and phone. Ask "
                 "each person first - a short email is fine.")
    steps.append("<strong>Submit and save the confirmation (1 min).</strong> Screenshot or note the confirmation "
                 "number and date in the tracker at the bottom of this page. The reply you get should come from the "
                 "employer's own email domain.")
    steps.append("<strong>Prepare and follow up.</strong> Rehearse a 30-second pitch: B.S. Chemistry UCSC 2011, HPLC "
                 "and purification work at Threshold, QC and records discipline at MicroConstants and Quintara, the "
                 "ACS J. Med. Chem. publication. Then two specific examples of careful work and two questions about "
                 "the lab's instruments and volume. If nothing comes back in 7-10 business days, send the short intro "
                 "email below or one polite portal message.")
...
def build_verify(job):
    items = [
        ...
        ("<strong>Documents</strong>", "Resume, cover letter and intro email on this page contain only r&eacute;sum&eacute; facts "
                                        "plus the wording of this posting. No commute, transit or address information appears in them; "
                                        "that stays in the &quot;Getting there&quot; section."),
    ]
...
def subpage_html(job, count):
    ...
    monitor = job.get("status") == "monitor"
    ...
    if job.get("status") == "flag":
        ...
        cls = "callout" if monitor else "callout ok"
    ...
    transit = ("From: 21st Ave & Judah St, San Francisco, CA 94122 (Judah & 21st Ave, N Judah)\n"
               ...
               % (esc(job.get("location", "")), esc(job.get("route", "")),
                  esc(job.get("company", "")), esc(job.get("officialLink", ""))))
    html_out = TEMPLATE
    reps = {
        "@@ID@@": job["id"],
        "@@COUNT@@": str(count),
    ...
    html_out = html_out.replace("@@TRANSIT@@", transit)
...
def main():
    jobs = load_jobs()
    if not jobs:
        sys.exit("no jobs parsed")
    # attach internal hints for pass-6 rows if present
    try:
    ...
    # Pass-6 self-audit rewrote the three City rows; their letter hints live there.
    try:
    ...
    n = 0
    for j in jobs:
        num = int(re.search(r"(\d+)$", j["id"]).group(1))
        tag = "job-%02d" % num
        rt = resume_text(j)
        ct = cover_text(j)
        et = email_text(j)
        open(os.path.join(RESUME_DIR, "%s_resume.txt" % tag), "w", encoding="utf-8").write(rt)
        make_pdf(rt, os.path.join(RESUME_DIR, "%s_resume.pdf" % tag))
        open(os.path.join(COVER_DIR, "%s_cover.txt" % tag), "w", encoding="utf-8").write(ct)
        make_pdf(ct, os.path.join(COVER_DIR, "%s_cover.pdf" % tag), title="")
        open(os.path.join(COVER_DIR, "%s_email.txt" % tag), "w", encoding="utf-8").write(et)
        page = subpage_html(j, count)
        open(os.path.join(JOBS_DIR, "%s.html" % tag), "w", encoding="utf-8").write(page)
        # keep the subpage path in sync with the actual file name
        j["subpage"] = "jobs/%s.html" % tag
        n += 1
    print("regenerated %d rows (resume/cover/email txt+pdf + subpages)" % n)
