# -*- coding: utf-8 -*-
"""JobSearchSF - clean professional document writer + subpage generator - PASS 7 Easy Apply.
...
Regenerates, for EVERY job row in assets/js/data.js:
  ...
  * jobs/job-NN.html                                  (10-step apply guide + Easy Apply box)

Rules enforced:
  - resumes / cover letters / emails contain NO transportation or commute content
  - no meta-language, no match-score footers, no placeholders
  - only facts that appear in master resume are claimed
  - NEW in Pass 7: Easy Apply box with Kit ZIP, DOCX, autofill vault
"""
import os
import re
import sys
...
<body>
  ...
  <main class="wrap">
    ...
    <section class="block" id="easy">
      <div class="easy-box">
        <h3>⚡ Easy Apply — Download & Submit in 3 Clicks (Pass 7)</h3>
        <p style="margin:0 0 8px"><strong>Goal:</strong> No typing, no re-formatting. One ZIP = everything you need. Direct official link only, no recruiters.</p>
        ...
        <div class="callout blue" style="margin:10px 0"><strong>3-Click Flow:</strong> 1) Download Kit ZIP (contains resume PDF+TXT+DOCX, cover PDF+TXT+DOCX, email TXT, screening answers, profile JSON, README with direct link) → 2) Open Official Apply → 3) Upload PDFs/DOCXs + copy-paste from Screening_Answers.txt → Submit. Save confirmation in tracker below.</div>
        <div class="copy-box">
    ...
    <section class="block" id="apply">
      <div class="card">
        ...
        <p class="sec-sub">Ten short steps, but Pass 7 makes it 3 clicks if you use the Easy Apply box above.</p>
        @@STATUS_CALL@@
...
@@STEPS@@
        ...
        <div class="callout blue">
          <strong>Copy-paste tip:</strong> use the Copy button on each document below, or just use the Kit ZIP from the Easy Apply box — it already contains everything.
        </div>
    ...
    <section class="block" id="email">
      <div class="card">
        ...
        <p class="sec-sub">Use only if official posting lists a contact address or portal has a message box. Kit includes this file.</p>
    ...
    <section class="block" id="verify">
      <div class="card">
        ...
        <p class="sec-sub">Nothing inferred. Official sources only, with direct links for manual review.</p>
        <ul class="clean">
@@VERIFY@@
        </ul>
    ...
    <footer>
      <p>JobSearchSF - @@ID@@ - @@COMPANY_ESC@@ - verification snapshot <span id="today"></span>. Apply through official employer portal only. No recruiters, no fees, no guesses. Pass 7 Easy Apply: Kit ZIP = download + submit.</p>
      <p><a href="../index.html">&larr; Back to main list</a> | <a href="@@OFFICIAL@@" target="_blank" rel="noopener">Official careers page</a> | <a href="@@APPLY_URL@@" target="_blank" rel="noopener">Apply direct</a> | <a href="../assets/kits/@@ID@@_kit.zip" download>📦 Kit ZIP</a></p>
    </footer>
  </main>
...
def build_autofill(job):
    # condensed autofill for this specific job
    rt = role_type(job)
    comp = clean_company(job.get("company", ""))
...
def build_steps(job):
    ...
    steps.append("<strong>Use Easy Apply box above (1 min).</strong> Click 'Download Kit ZIP' — it contains resume PDF+TXT+DOCX, cover PDF+TXT+DOCX, email TXT, autofill answers, profile JSON, README with direct link. No extra formatting needed.")
    steps.append("<strong>Open Official Apply (1 min).</strong> Click 'Open Official Apply →' — official employer ATS only (Greenhouse, Lever, Ashby, Workday, SmartRecruiters, UltiPro, AP Recruit). Verify live.")
    ...
    steps.append("<strong>Answer screening honestly (2 min).</strong> Role core: %s. If technique not done, answer No and mention what you HAVE done — HPLC, NMR, UV-Vis, chromatography, sample prep, GLP, QC, electronic records."
                 % esc(req_short))
...
def build_verify(job):
    items = [
        ...
        ("<strong>Kit ZIP (download & submit)</strong>", "<a href=\"../assets/kits/%s_kit.zip\" download>../assets/kits/%s_kit.zip</a> — contains resume PDF+TXT+DOCX, cover PDF+TXT+DOCX, email TXT, screening answers, profile JSON, README" % (esc(job.get("id","")), esc(job.get("id","")))),
...
def main():
    ...
    for j in jobs:
        ...
        tag = "job-%02d" % num if num < 100 else f"job-{num}"
        # For IDs like job-01, tag matches. For job-100, tag job-100.
        # Ensure tag matches actual ID for file naming consistency: use ID directly
        tag_file = j["id"]  # job-01 etc, job-100 etc
        rt = resume_text(j)
        ct = cover_text(j)
        et = email_text(j)
        ...
        make_pdf(ct, os.path.join(COVER_DIR, "%s_cover.pdf" % tag_file), title="")
        ...
        j["subpage"] = "jobs/%s.html" % tag_file
        n += 1
    print("regenerated %d rows (resume/cover/email txt+pdf + subpages) Pass7" % n)
